...
```bash
oft trace doc/system_requirements.md doc/design.md src/main/java src/test/java
```
...
```bash
oft trace -a feat,req,dsn doc/system_requirements.md doc/design.md src/main/java src/test/java
```
...
```bash
oft trace -v ALL doc/system_requirements.md doc/design.md src/main/java src/test/java
```

And if we want to find a specific requirement in that trace?

```
oft trace -v ALL doc/system_requirements.md doc/design.md src/main/java src/test/java | grep '^[^|].*req~tracing.deep-coverage~1'
```
...
```bash
id='req~tracing.deep-coverage~1' oft trace -v ALL doc/system_requirements.md doc/design.md src/main/java src/test/java | grep -Pzo 'ok.*'"$id"'.*\n([|].*\n)+'
```
...
```bash
oft convert -o specobject doc/system_requirements.md doc/design.md
```

*(The next step assumes you have `xmllint` installed. Of course you can use other software to pretty print the XML output too)*

```bash
oft convert -o specobject doc/system_requirements.md doc/design.md | xmllint --format - > /tmp/oft_specifications.xml
```
